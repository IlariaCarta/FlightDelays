...
public class Model {
	...
	public void creaGrafo(int distanzaMedia) {
		...
		for(Rotta rotta: dao.getRotte(aIdMap, distanzaMedia)) {
			...
			if(edge==null) {
				...
				// Il grafo in questo caso non è orientato, perciò bisogna aggiornare i pesi quando si trovano le due 
				// direzioni diverse
			}
	...
	public Boolean testConnessione(Integer a1, Integer a2) {
		...
		//mi serve tenere traccia degli ereoporti che ho visitato
		...
		//supponiamo di scegliere in ampiezza
		BreadthFirstIterator<Airport, DefaultWeightedEdge> it = new BreadthFirstIterator<>(this.grafo, partenza);
		//visita in ampiezza a partire dal vertice partenza -> dovremmo trovare quello con il numero minimo di archi attraversati
		while(it.hasNext()) {
